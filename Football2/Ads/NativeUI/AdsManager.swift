...
class AdsManager: NSObject {
    ...
    func showInterstitialAd() {
        ...
        if isInterstitialAdReady() {
            ...
            print("❌ Interstitial ad not ready, loading now...")
            loadInterstitialAd()
        }
    }
...
//            }
    ...
    func presentInterstitialAd2(vc:UIViewController, completion: @escaping () -> Void) {
        if Subscribe.get() {
            completion()
            return
        }
...
extension AdsManager: FullScreenContentDelegate {
    ...
    func ad(_ ad: FullScreenPresentingAd, didFailToPresentFullScreenContentWithError error: Error) {
        ...
        // POST NOTIFICATION for ad failure
        NotificationCenter.default.post(name: .interstitialAdDidFail, object: nil)
    ...
    func adDidDismissFullScreenContent(_ ad: FullScreenPresentingAd) {
        ...
        // POST NOTIFICATION for ad dismissal
    ...
    // ... keep other delegate methods as they are ...
}
...
class GoogleNativeAds: NSObject, NativeAdLoaderDelegate {
    ...
    let googleNativeAdsCustomeViews: GoogleNativeAdsCustomeViews = GoogleNativeAdsCustomeViews.instanceFromNib() as! GoogleNativeAdsCustomeViews
    /// Load Media native ads
    func showAdsViews(nativeAd: NativeAd, view: UIView) {
        view.isHidden = false
        displaySubViewtoParentView(view, subview: googleNativeAdsCustomeViews)
        googleNativeAdsCustomeViews.nativeAd = nativeAd
        googleNativeAdsCustomeViews.setup()
    }
